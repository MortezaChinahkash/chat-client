...
export interface KeypairB64 { pub: string; priv: string; }
...
export class CryptoService {
  ...
  async init() {
    if (!this.ready) { await sodium.ready; this.ready = true; }
    const cached = localStorage.getItem('kp');
    if (cached) {
      this.me = JSON.parse(cached);
    } else {
      const kp = sodium.crypto_box_keypair();
      this.me = { pub: sodium.to_base64(kp.publicKey), priv: sodium.to_base64(kp.privateKey) };
